...
function void ram_tb::build_phase(uvm_phase phase);
  ...
  if (m_cfg.has_wagent) begin
    ...
    foreach (wagt_top[i]) begin
      ...
      uvm_config_db#(ram_wr_agent_config)::set(this, $sformatf("wagt_top[%0d]*", i),
                                               "ram_wr_agent_config", m_cfg.m_wr_agent_cfg[i]);
  ...
  if (m_cfg.has_ragent == 1) begin
    ...
    foreach (ragt_top[i]) begin
      ...
      uvm_config_db#(ram_rd_agent_config)::set(this, $sformatf("ragt_top[%0d]*", i),
                                               "ram_rd_agent_config", m_cfg.m_rd_agent_cfg[i]);
      // Create the instances of wagt_top[i] handles
...
function void ram_tb::connect_phase(uvm_phase phase);
  if (m_cfg.has_virtual_sequencer) begin
    ...
    if (m_cfg.has_wagent) foreach (wagt_top[i]) v_sequencer.wr_seqrh[i] = wagt_top[i].agnth.seqrh;
    if (m_cfg.has_ragent) foreach (ragt_top[i]) v_sequencer.rd_seqrh[i] = ragt_top[i].agnth.seqrh;
  end

  // connect the corressponding analysis port of all the
  // write & read agents to the scoreboard's analysis export
  if (m_cfg.has_scoreboard) begin
    foreach (wagt_top[i])
